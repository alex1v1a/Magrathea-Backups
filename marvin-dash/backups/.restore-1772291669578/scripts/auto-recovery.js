...
async function detectPortConflict(port) {
  ...
  try {
    // Use PowerShell-native Get-NetTCPConnection instead of netstat/findstr
    // This works reliably in PowerShell environments
    let connections = [];
    try {
      const { stdout } = await execPromise(
        `powershell.exe -Command "Get-NetTCPConnection -LocalPort ${port} -ErrorAction SilentlyContinue | Select-Object LocalAddress, LocalPort, State, OwningProcess | ConvertTo-Json -Compress"`,
        { timeout: 10000 }
      );
      if (stdout.trim()) {
      ...
      // Fallback to cmd.exe /c for netstat if PowerShell fails
      log(`    PowerShell detection failed, trying CMD fallback...`, 'dim');
      try {
        const { stdout } = await execPromise(`cmd.exe /c "netstat -ano | findstr :${port}"`, { timeout: 10000 });
        ...
        for (const line of lines) {
          ...
          if (localPort === String(port)) {
            connections.push({
              ...
              LocalPort: port,
              State: parts[3],
              OwningProcess: parts[parts.length - 1]
            });
          }
        }
      } catch (cmdError) {
        throw new Error(`Both PowerShell and CMD detection failed`);
      }
    }
    ...
    for (const conn of connections) {
      const pid = conn.OwningProcess || conn.pid;
      const localAddress = conn.LocalAddress || `0.0.0.0:${conn.LocalPort}`;
      const state = conn.State || 'UNKNOWN';
      
      if (pid && !isNaN(parseInt(pid))) {
        try {
          // Get process name using PowerShell
          const { stdout: processOutput } = await execPromise(
            `powershell.exe -Command "Get-Process -Id ${pid} -ErrorAction SilentlyContinue | Select-Object ProcessName | ConvertTo-Json -Compress"`,
            { timeout: 5000 }
          );
