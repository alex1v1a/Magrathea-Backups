...
try {
  const mem = execSync('powershell -Command "Get-CimInstance Win32_OperatingSystem | Select-Object TotalVisibleMemorySize,FreePhysicalMemory | Format-Table"', { encoding: 'utf8' });
  console.log(mem);
} catch {}
...
if (recentDumps.length > 3) {
  ...
  console.log('   - Kill Chrome: taskkill /F /IM chrome.exe');
  console.log('   - Delete: %LOCALAPPDATA%\Google\Chrome\User Data\');
  console.log('   - Delete: C:\Users\Admin\.openclaw\chrome-marvin-only-profile\');
  console.log('   - Reinstall Chrome if needed');
  
  console.log('\n2. PROFILE RESET - Create completely new profile:');
  console.log('   - Use chrome --user-data-dir=C:\\temp\\chrome-fresh');
